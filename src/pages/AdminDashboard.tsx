    ...
    const fetchData = async () => {
      ...
      try {
        ...
        try {
          ...
          // Update API status based on successful fetch
          ...
          if (kycData.length > 0) {
            kycData = kycData.map(kyc => {
              // If status exists but is not in the expected format, normalize it
              if (kyc.status && typeof kyc.status === 'string') {
                const status = kyc.status.toUpperCase();
                // Ensure it's one of our valid enum values
          ...
          kycData = []; // Ensure empty array on error
        }
        ...
        <TabsContent value="ambassadors" className="space-y-4">
          {!apiStatus.ambassadors && (
            <Alert variant="warning" className="mb-4">
              ...
              <AlertDescription>
                The ambassador management API is not fully implemented yet.
              </AlertDescription>
            </Alert>
          )}
          <AmbassadorList
        ...
        <TabsContent value="receipts">
          {!apiStatus.receipts && (
            <Alert variant="warning" className="mb-4">
              <AlertTriangle className="h-4 w-4" />
              <AlertDescription>
                The receipt management API is not fully implemented yet.
              </AlertDescription>
            </Alert>
          )}
          <ReceiptList
        ...
        <TabsContent value="kyc">
          ...
          {!apiStatus.kyc ? (
            <Alert variant="warning" className="mb-4">
              <AlertTriangle className="h-4 w-4" />
              <AlertDescription>
                The KYC management API is not implemented yet. This feature will be available soon.
              </AlertDescription>
            </Alert>
          ) : (
