...
export const login = async (
  ...
  try {
    ...
    console.log("Making API call to login user:", { email: data.email });
    }
    
    // Make the login request to get custom token
    ...
    // Exchange custom token for ID token using Firebase SDK
    try {
      if (import.meta.env.DEV) {
      ...
      // Store both tokens for debugging and compatibility
      localStorage.setItem('firebaseToken', idToken);
      localStorage.setItem('backendToken', customToken); // Store the custom token too
      localStorage.setItem('accessToken', idToken); // Primary access token
      
      if (import.meta.env.DEV) {
      console.log("Token exchange successful");
      console.log("Firebase UID:", userCredential.user.uid);
      console.log("Backend UID:", response.data.uid);
      }
      
      // Update the response with the ID token
      response.data.accessToken = idToken;
    } catch (firebaseError: any) {
      console.error("Firebase token exchange error:", firebaseError);
      
      // If Firebase token exchange fails, still keep the custom token
      ...
      // We'll continue using the custom token, but log the error
      console.warn("Using custom token as fallback due to Firebase token exchange error");
    }
    ...
    if (error.error) {
      // If we've already created an error object, just rethrow it
      throw error;
      ...
      throw { 
        error: 'network-error', 
        message: 'Unable to connect to the server. Please check if the server is running or your internet connection.' 
      };
...
export const registerAmbassador = async (
  ...
  try {
    console.log("Making API call to register ambassador:", data);
    
    // Using the correct endpoint
    console.log("Using endpoint: /ambassador");
...
export const registerAdmin = async (
  ...
  try {
    console.log("Making API call to register admin:", data);
    
    // Using the correct endpoint
    console.log("Using endpoint: /admin");
