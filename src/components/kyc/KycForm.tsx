...
export default function KYCForm({ ambassadorId }: KYCFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log("Authentication check - Current user:", auth.currentUser);
      console.log("Authentication check - Ambassador ID:", ambassadorId);
      ...
      // Create FormData object to hold files and form data
      const formDataObj = new FormData();
      
      // Always use the Firebase auth UID which must match the backend record
      formDataObj.append('ambassadorId', user.uid);
      console.log("Using Firebase UID for KYC submission:", user.uid);
      ...
      if (idFrontFile) {
        formDataObj.append('idFront', idFrontFile);
      }
      
      if (idBackFile) {
        formDataObj.append('idBack', idBackFile);
      }
      
      // Submit the KYC application using the API service
      ...
      if (error.response) {
        ...
        if (error.response.status === 404) {
          errorMessage = "The KYC submission service is currently unavailable. Please try again later.";
          ...
          errorMessage = "The uploaded files are too large. Please resize your images and try again.";
